...
let ballColors = [];
for(let i=0; i<numBalls; i++)  ballColors[i] = '#' + Math.floor(Math.random()*16777215).toString(16);
...
function drawConcaveArc(p1, p3, center, ctx){
    ...
    ctx.arc(center.x, center.y, ConcavenessRadius, startAngle, endAngle, anticlockwise = true );
    ctx.stroke();
}
...
function initBoudaries(ctx){
    ...
    let centreOffset = Math.sqrt(ConcavenessRadius*ConcavenessRadius - Math.pow(Math.abs(box2[0][0]-box2[1][0])/2,2));
    drawConcaveArc({x: box2[0][0], y: box2[0][1]}, {x: box2[1][0], y: box2[1][1]}, {x: (box2[0][0] + box2[1][0])/2, y: box2[0][1] - centreOffset} ,ctx);
    drawConcaveArc({x: box2[1][0], y: box2[1][1]}, {x: box2[2][0], y: box2[2][1]}, {y: (box2[1][1] + box2[2][1])/2, x: box2[1][0] + centreOffset} ,ctx);
    drawConcaveArc({x: box2[2][0], y: box2[2][1]}, {x: box2[3][0], y: box2[3][1]}, {x: (box2[2][0] + box2[3][0])/2, y: box2[2][1] + centreOffset} ,ctx);
    drawConcaveArc({x: box2[3][0], y: box2[3][1]}, {x: box2[0][0], y: box2[0][1]}, {y: (box2[0][1] + box2[3][1])/2, x: box2[0][0] - centreOffset} ,ctx);
}

function initBalls(){
    // console.log(width * 0.7,height * 0.7);
    let [x0,y0] = [box1[0][0] + (initGroupCircleRadius + 15), box1[0][1] + (initGroupCircleRadius + 30)];
    nCircumBalls([x0, y0], initGroupCircleRadius, numBalls, ballsNormal);

    [x0,y0] = [box2[0][0] + (initGroupCircleRadius + 15), box2[0][1] + (initGroupCircleRadius + 30)];
...
function sizeCanvas() {
    ...
    box1 = [ [w * (2/20),h * 0.1], [w * (8/20), h* 0.1] , [w * (8/20),h * 0.9] , [w * (2/20), h * 0.9]];
    box2 = [ [w * (12/20),h * 0.1], [w * (18/20), h* 0.1] , [w * (18/20),h * 0.9] , [w * (12/20), h * 0.9]];
}
